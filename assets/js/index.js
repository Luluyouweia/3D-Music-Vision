...
function main() {
    ...
    function audioManager() {
        if (isPlayed) {
            ...
            dataArray.forEach((e, index) => {
                ...
                tempSum = e;
                if ( index >=16) {
                    targetArray.push(2+tempSum );
                    
                    tempSum = 0;
                }
            })
            for (let i = 0; i < 55; ++i) {
                keys[i].scale.y = 450 * Math.sin(Math.PI / 2 * (targetArray[i] / 256));
            }
            /*弃用算法
            for (let i = 0; i < 55; ++i) {
                keys[i].scale.y = targetArray[i] ;
                //keys[i].scale.y = 450 * (Math.tan(Math.PI /2 * (Math.sin(Math.PI/2*targetArray[i] / 256))-Math.PI/3)+1.7);
            }*/
            a = dataArray;
        }
    }
    ...
    */
    ...
    const universe = (() => {
        ...
        const bg = new THREE.TextureLoader().load("./assets/texture/"+ihdr);
    ...
    function render(time) {
        ...
        if (locks[0]) {
            ...
            if (locks[2]) {
                if (stageRotate.position.y >= -55) {
                    camera.position.set(100 * Math.sin(time * 0.5), time, 64 * Math.cos(time * 0.5));
                    camera.lookAt(0, 60+time*0.5, 0);
                    //controls.rotation.x = Math.PI/5;
                    stageRotate.position.y -= 0.1;
                    ...
                    setTimeout(() => {
                        ...
                        camera.lookAt(-64, 36, 0);
                        setTimeout(() => {
                            camera.position.set(-64, 25, 16);
                            camera.lookAt(-32, 25+time, 0);
                            setTimeout(() => {
                                camera.position.set(-256, 15, 0);
                                setTimeout(() => {
                                    camera.position.set(-100, 8, -64);
